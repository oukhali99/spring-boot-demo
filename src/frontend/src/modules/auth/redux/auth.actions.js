...
export const register = (username, password) => async (dispatch, getState) => {
    ...
    const resJson = await dispatch(
        apiActions.request("http://localhost:8080/api/v1/auth/register", requestOptions)
    );
...
export const authenticate = (username, password) => async (dispatch, getState) => {
    ...
    const resJson = await dispatch(
        apiActions.request("http://localhost:8080/api/v1/auth/authenticate", requestOptions)
    );
