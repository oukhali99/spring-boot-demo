...
class HoursColumn extends React.Component {
    render() {
        ...
        let hours = [
            <div key="ground" style={{ borderStyle: "solid", height: "30px", textAlign: "center" }}>
                Hour
            </div>,
        ];
        ...
        let weekdays = [
            <div
                ...
                style={{
                    ...
                    textAlign: "center",
                    backgroundColor:
                        this.props.highlightedWeekday === this.props.weekday
                            ? highLightedColor
                            : "",
...
class HourCell extends React.Component {
    ...
    async onClick() {
        ...
        const available = availabilities.some(
            (availability) => availability.hour === hour && availability.weekday === weekday
        );
        const action = !available ? "add" : "remove";

        // Make the call
        const res = await axios.post(
            process.env.REACT_APP_SERVER_URL + "/availabilities/" + action,
            body
        );
    ...
    render() {
        ...
        const userIsAvailable = availabilities.some(
            (availability) => availability.hour === hour && availability.weekday === weekday
        );
        ...
        if (this.props.matchMode !== 0) {
            ...
            if (this.props.matchMode === 1) {
                ...
                friendIsAvailable = friendAvailabilities.some(
                    (availability) => availability.hour === hour && availability.weekday === weekday
                );
                ...
                for (var i = 0; i < groupAvailabilities.length; i++) {
                    ...
                    const memberAvailable = memberAvailabilities.some(
                        (availability) =>
                            availability.hour === hour && availability.weekday === weekday
                    );
...
class Availabilities extends React.Component {
    ...
    async matchWithFriend() {
        ...
        const res = await axios.post(
            process.env.REACT_APP_SERVER_URL + "/friends/getFriendAvailabilities",
            body
        );
        console.log(res.data.message);
        if (!res.data.success) {
    ...
    async matchWithGroup() {
        ...
        const res = await axios.post(
            process.env.REACT_APP_SERVER_URL + "/groups/getAvailabilities",
            body
        );
        console.log(res.data.message);
        if (!res.data.success) {
