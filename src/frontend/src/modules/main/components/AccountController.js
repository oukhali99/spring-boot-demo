...
class AccountController extends React.Component {
    ...
    async logoff() {
        ...
        const res = await axios.post(process.env.REACT_APP_SERVER_URL + "/users/logoff", {
            userId,
            sessionId,
        });
...
class LogButton extends React.Component {
    render() {
        if (!this.props.loggedIn) {
            return <Link to="/login">Register / Login</Link>;
        }

        return (
            <Link onClick={this.props.logoff} to="/">
                Logoff
            </Link>
        );
    }
...
class SessionClock extends React.Component {
    ...
    async updateSessionFromAPI() {
        ...
        const body = {
            sessionId: this.props.sessionState.sessionId,
            userId: this.props.sessionState.userId,
        };

        // Do the call
        const res = await axios.post(
            process.env.REACT_APP_SERVER_URL + "/users/getUserDocument",
            body
        );
    ...
    async isValidSession() {
        ...
        const res = await axios.post(
            process.env.REACT_APP_SERVER_URL + "/users/isUserSessionValid",
            { sessionId, userId }
        );
    ...
    render() {
        ...
        let formattedTimeTilTimout_s = Math.floor(timeTilTimout_s)
            ? Math.floor(timeTilTimout_s)
            : "Loading...";
